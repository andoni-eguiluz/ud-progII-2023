...
/** Clase Circulo corregida para evitar el error en los choques (provocado por el tipo int en vez de double al incorporar la velocidad)
 * @author andoni.eguiluz at ingenieria.deusto.es
 */
public class CirculoCorregido {
	...
	public static void setGrosor(float grosor) {
		CirculoCorregido.grosor = grosor;
	}
	...
	private double xCentro;  // Progresadas a dobles para cálculo exacto de velocidad
	private double yCentro;  //  Observa cómo se puede cambiar la implementación de los atributos sin que afecte a las clases usuarias porque el método (set,get) no cambia
	 ...
	 */
	public CirculoCorregido(int radioEnPixels, int xCentro, int yCentro, Color colorBorde, Color colorRelleno) {
	 ...
	 * @param colorRelleno	Color de rellenoe
	 */
	public CirculoCorregido(int radioEnPixels, Point centro, Color colorBorde, Color colorRelleno) {
...
//		tiempoCreacionMs = System.currentTimeMillis();
	 ...
	 * @param yCentro	Coordenada y del centro del círculo, en píxeles (de arriba abajo)
	 */
	public CirculoCorregido(int radioEnPixels, int xCentro, int yCentro) {
...
//		tiempoCreacionMs = System.currentTimeMillis();
	 ...
	 */
	public CirculoCorregido( VentanaGrafica ventana ) {
		radioEnPixels = RADIO_MINIMO + random.nextInt( RADIO_MAXIMO-RADIO_MINIMO+1 ); // 0-10 + 10
